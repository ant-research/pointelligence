...
reusing the input one. Coordinate generation is the defining design
choice, so it is a swappable strategy:
...
- :class:`KernelStampGenerator` — the deterministic implementation:
  each input point stamps the K kernel-tap offsets, and the output set
  is the deduplicated voxel-center union.
...
class GeneratedSites:
    ...
    # When the rulebook is k-sorted with EXACTLY this many
    ...
    # True iff the dedup merged nothing (n_out == N*K) — the
...
class CoordinateGenerator(Protocol):
    ...
    .. note::
        **Provisional API.** This strategy protocol
