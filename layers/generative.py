...
reusing the input one (a generative-expansion conv, vs a submanifold
conv that keeps the input sites). Coordinate generation is the defining
design choice, so it is a swappable strategy:
...
- :class:`KernelStampGenerator` — the deterministic implementation:
  ...
  the deduplicated voxel-center union. The ``SparseConv3d``
  generative-mode analog.
...
class GeneratedSites:
    ...
    # cyc-18 T7: when the rulebook is k-sorted with EXACTLY this many
    # triplets per kernel tap (the stamp construction under sort_by="k":
    # every input stamps every tap), the TIG index path goes closed-form
    # (no sortedness sync, no searchsorted, no cumsum). None = unknown.
    uniform_seg_len: Optional[int] = None
    # cyc-18 T7b: True iff the dedup merged nothing (n_out == N*K) — the
    # stamping was injective, so every output site is generated by exactly
    # ONE (input, tap): the fan-in-1 subdivision-upsample contract. Routes the FI1
    # plain-store forward. Canonical instance: the subdivision generator
    # (ks=2, expansion=2 on voxel-centered inputs).
    ...
    def to_contract(self) -> TripletContract:
        """The structural contract for this generated rulebook.

        Replaces the ad-hoc ``tig_hint`` dict GenerativePointConv3d used to
        build inline: the generator already KNOWS these facts at construction
        (k-sorted stamp layout, closed-form uniform segments, injective-cover),
        so they ship as the same typed carrier every other conv path uses. No
        host reduction — ``uniform_seg_len``/``exact_cover_out`` are structural
        identities of the stamp build (generative.py:318/321).
        """
        return TripletContract(
            k_sorted=True,
            exact_cover_out=self.exact_cover_out,
            exact_cover_in=False,
            uniform_seg_len=self.uniform_seg_len,
        )
...
class CoordinateGenerator(Protocol):
    ...
    .. note::
        **Provisional API (Goal-03 Sub-goal G).** This strategy protocol
        is the locus of future generative-conv research — the staged
        strided-down-conv and learnable-transpose members,
        plus learned-occupancy / deformable generators, may reshape this
        interface and :class:`GeneratedSites`. Treat the signature as
        unstable until the operator family stabilises; the operator
        ``GenerativePointConv3d`` and its parity contract are stable.
    """
...
def build_generative_triplets(
    ...
    with torch.no_grad():
        ...
        if sort_by == "k":
            ...
            # That single torch.sort was ~44% of the enc0 builder.
    ...
    return GeneratedSites(
        ...
        # segment contract the TIG index consumes (cyc-18 T7).
        uniform_seg_len=(N if sort_by == "k" else None),
        # T7b: dedup merged nothing ⇒ injective stamping ⇒ fan-in-1
...
class KernelStampGenerator:
    """Deterministic coordinate generator — the ``SparseConv3d`` analog.
...
class SubdivisionGenerator:
    """Generation-from-ONE — the ``SparseInverseConv3d`` / unpatchify
    analog (cyc-18 T7b).
