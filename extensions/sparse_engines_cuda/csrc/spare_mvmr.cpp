...
at::Tensor
	...
	// the shared check now admits fp16/bf16 for the CUDA
	// path, but this CPU reference impl reads `data_ptr<float>()` directly,
	// so it stays fp32-only with an explicit guard (low-precision CPU conv
	// has no caller — the CUDA kernel handles fp16/bf16).
	TORCH_CHECK(a.dtype() == torch::kFloat32,
		"sparse_matrix_vector_multiplication_reduction (CPU): fp32 only; "
