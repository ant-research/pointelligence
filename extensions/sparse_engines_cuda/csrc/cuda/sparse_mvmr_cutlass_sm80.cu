// G22 Task M1 — single (M_TILE, S_TILE) CUTLASS GEMM for mvmr (affine).
// Per the G22 plan (g22_cutlass_mvmr_closure_plan.md) §4 Task-M1 GO/NO-GO.
//
// One host entry point over the templated kernel body
// (MvmrCutlassSm80SingleTileOp in the .cuh):
//
//   M1 — sparse_mvmr_cutlass_sm80_single_tile(W_seg, B_seg, C_seg_padded)
...
//
// M2 adds the kernel-side b_idx IndexedGather on the input operand; M3
// adds the outer (k, m-tile, ...) grid + the scatter-accumulate-by-a_idx
// epilogue. M1 = dense-write epilogue, no gather, no scatter — it proves
// the GEMM core + the contraction-axis-C orientation only.
...
// via setup.py — same include the vvor Task-2 path uses).
...
at::Tensor sparse_mvmr_cutlass_sm80_single_tile(
  ...
  TORCH_CHECK(W_seg.scalar_type() == at::kHalf && B_seg.scalar_type() == at::kHalf,
      "sparse_mvmr_cutlass_sm80: M1 supports fp16 only");
...
// M2 moves that gather INSIDE the CUTLASS mainloop via a composed
// `IndexedGather` custom-stride layout on B, mirroring the proven vvor
// Task-2 path (sparse_vvor_cutlass_sm80.cu :: ..._single_tile_gathered).
//
// ── Why mvmr's B-gather does NOT need vvor Task-2's transposing 2nd Config ──
//
// vvor Task 2 gathered along the *contraction* axis K: its B view
// (N_TILE, K_seg) has stride (_1, C_full) so the first non-unit stride —
// the one make_gather_tensor replaces — is mode-1 = K = the contraction
// axis. With the gathered dim ON the contraction axis, consecutive
// K-elements live in disparate gmem rows, so the K-vector cp.async
// (`Layout<Shape<_1,_8>>`) cannot compose with the gather tensor (CuTe
// Copy_Traits correctly rejects the vectorization). vvor resolved this
// with a 2nd Config: M/N-major smem + a transposing SM75_U16x8_LDSM_T
// smem-copy atom (the cycle-4 §1.11 Task-2 pitfall, ~hours).
//
...
// exactly as in M1's affine B. The cp.async vector still runs along the
// contiguous C axis (M1's `(_32,_4)×(_1,_8)` GmemTiledCopyB, the
// G14-validated layout); the gather only re-points the per-S-row base
// address. Smem stays C-contig (LDSM_N), the TN MMA atom is unchanged.
// So the M1 MvmrCutlassSm80FpropConfig + MvmrCutlassSm80SingleTileOp
// compose directly with the gather tensor — no transposing 2nd Config.
//
// Padded-K caveat (inherited from vvor Task-2): C_seg is padded by the
// host to a TileK multiple with zeros on W_seg's channel axis; B is
// read from `input_b` whose C axis already spans C_full ≥ C_TILE, so the
// padded channels read real-but-unused input values — they are masked to
// zero by W_seg's zero-padded channels (W[m, c]=0 for padded c ⇒ the
// product is exactly 0). The S-axis padded slots (b_idx sentinel rows
...
// values, identical to the vvor Task-2 contract.
...
void mvmr_cutlass_sm80_single_tile_gathered_kernel(
  ...
  // W stays affine, exactly as M1: (M_TILE, C_seg) row-major, C-contig.
  auto sA = make_shape(MvmrConfig::TileM{}, C_seg);
  auto dA = make_stride(C_seg, _1{});
  Tensor mA = make_tensor(make_gmem_ptr(W_ptr), make_layout(sA, dA));
  ...
  // Output is plain affine (M_TILE, S_TILE) row-major fp32 (M1 dense write).
  auto sC = make_shape(MvmrConfig::TileM{}, MvmrConfig::TileN{});
  auto dC = make_stride(MvmrConfig::TileN{}, _1{});
  Tensor mC = make_tensor(make_gmem_ptr(O_ptr), make_layout(sC, dC));
...
// over ceil(seg_len / S_TILE) S-chunks; per chunk it builds the M2-style
...
//
// Padded-S resolution (G24-T2): kernel-side virtual zero, no host
// sentinel row. SegmentClampedGather clamps any chunk S-slot whose
// in-chunk offset ≥ chunk_len to in-bounds real row 0; that column's
// GEMM value is computed but the scatter loop stops at the real
// chunk_len so it is never emitted into o. OOB contribution is thus
// exactly 0 by structural exclusion — bit-identical to the prior
// Option-3 host-zero-row (which only ever zeroed those same unused
// columns), with the per-call host (N_b+1,W) alloc/copy/zero_ removed.
// Arbitrary / empty seg_len just work.
...
//
// ── G24-T2: kernel-side virtual-zero sentinel (drop the host (N_b+1,W) row) ──
//
// Before G24-T2 the host appended a physical all-zero row at index N_b
// (`at::empty({N_b+1,W})` + narrow-copy + 1-row `zero_`) and OOB slots
// gathered THAT row, so the GEMM column for an OOB S-slot evaluated to
// exactly 0.0. That host alloc + full-tensor narrow-copy was the S3 half
// of the ~170 µs per-call DirectCopy (G24-T1 §pin). It is removable
// *without* changing the OOB result by one bit, because the OOB column's
// GEMM value is **never read**:
//
...
// exclusion** — bit-identical to the prior Option-3 behavior, with the
...
void mvmr_cutlass_sm80_full_kernel(
  ...
  // (stride (C_full, 1)), exactly M1's affine A. We pass the padded
...
at::Tensor sparse_mvmr_cutlass_sm80_full(
  ...
  TORCH_CHECK(a.scalar_type() == at::kHalf && input_b.scalar_type() == at::kHalf,
      "sparse_mvmr_cutlass_sm80_full: fp16 only");
  ...
  // G24-T2: no host sentinel-zero-row. The kernel-side
  ...
  // the (N_b+1,W) at::empty/narrow-copy/zero_ (the S3 half of the
  // per-call ~170 µs DirectCopy) is deleted.
...
// ─── G25-T1 — repack-skipping host entry (`_full` minus the internal
...
// regardless of the caller's input layout. For the G26 fused conv
...
// an irreducible ~14 MB DtoD copy per iter (the post-fusion "S2"
// residual, G24-T6 / G26-T5).
//
...
// checks, seg_offs handling, the G24-T2 no-host-sentinel kernel-side
...
at::Tensor sparse_mvmr_cutlass_sm80_full_prestaged(
  ...
  TORCH_CHECK(aT.scalar_type() == at::kHalf && input_b.scalar_type() == at::kHalf,
      "sparse_mvmr_cutlass_sm80_full_prestaged: fp16 only");
  TORCH_CHECK(b_idx.scalar_type() == at::kInt && o_idx.scalar_type() == at::kInt,
  ...
  cudaError_t attr_err = cudaFuncSetAttribute(
      ...
      static_cast<int>(smem_size));
  TORCH_CHECK(attr_err == cudaSuccess,
      "cudaFuncSetAttribute (mvmr full prestaged) failed: ",
      cudaGetErrorString(attr_err));
  ...
  cudaError_t err = cudaGetLastError();
  TORCH_CHECK(err == cudaSuccess,
      "mvmr_cutlass_sm80_full_prestaged kernel launch failed: ",
      cudaGetErrorString(err));
